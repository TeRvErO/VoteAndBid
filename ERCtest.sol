...
interface UniswapV2Router02 is IUniswapV2Router01 {
...
contract BuyUniswap {
    ...
    address public contractETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address[] public path;
    address[] public path_reverse;
    uint public deadline = block.timestamp + 1 hours;
    uint[] public amounts;
    uint public quantityReceived;
    uint public amountOutMin;
    uint public amountOutMinReverse;
    uint public _amountETH;
    uint public receivedETHback;
    ...
    constructor() public {
        ...
        contractToken = ERC20(0xf8C3527CC04340b208C854E985240c02F7B7793f);
        contractTokenAddress = 0xf8C3527CC04340b208C854E985240c02F7B7793f;
    }
    
    function getAmountOut(ERC20 tokenOut, uint amount, uint price, uint threshold) internal view returns (uint amountOut){
        uint maxPrice = price / (1 - threshold);
        amountOut = uint(amount.mul(10 ** uint(tokenOut.decimals())).div(maxPrice));
    }

    function buy(uint price, uint amountETH, uint threshold) public {
        _amountETH = amountETH;
        amountOutMin = getAmountOut(contractToken, _amountETH, price, threshold);

        path.push(contractETH);
        path.push(contractTokenAddress);
        path_reverse.push(contractTokenAddress);
        path_reverse.push(contractETH);
        
        amounts = uniswap.swapExactETHForTokens(amountOutMin, path, address(this), deadline);
        quantityReceived = amounts[1];
        amountOutMinReverse = _amountETH.div(2);
        
        amounts = uniswap.swapExactTokensForETH(quantityReceived, amountOutMinReverse, path_reverse, address(this), deadline);
        receivedETHback = amounts[1];
